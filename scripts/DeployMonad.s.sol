...
contract DeployMonadMainnet is Script {
  ...
  function run() external {
    ...
    StaticATokenLM staticImpl = new StaticATokenLM(
      IPool(POOL),
      IRewardsController(REWARDS_CONTROLLER)
    );
    ...
    StaticATokenFactory factoryImpl = new StaticATokenFactory(
      IPool(POOL),
      address(proxyAdmin),
      ITransparentProxyFactory(address(proxyFactory)),
      address(staticImpl)
    );
    ...
    console.log('\nThen run:');
    console.log(
      'forge script scripts/VerifyDeployment.s.sol:VerifyDeployment --rpc-url monad -vvv'
    );
    console.log('');
  }
