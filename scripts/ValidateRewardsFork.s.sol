...
contract ValidateRewardsFork is Script {
  ...
  function _initContext() internal {
    ...
    try wrapper.REWARD_RESCUE_ADMIN() returns (address rescueAdmin) {
      console2.log('rewardRescueAdmin', rescueAdmin);
      require(rescueAdmin != address(0), 'RESCUE_ADMIN_ZERO');
    } catch {
      revert('RESCUE_ADMIN_UNREADABLE');
    }
  ...
  function _loadDustConfig() internal {
    ...
    try IDustRewardsControllerView(address(controller)).getTransferStrategy(dust) returns (
  ...
  function _assertConfig() internal view {
    console2.log('\nAsserting config');
    address expectedController = vm.envOr(
      'EXPECTED_CONTROLLER',
    ...
    require(
      keccak256(bytes(wrapper.name())) == keccak256(bytes(expectedName)),
      'WRAPPER_NAME_MISMATCH'
    );
    require(
      keccak256(bytes(wrapper.symbol())) == keccak256(bytes(expectedSymbol)),
      'WRAPPER_SYMBOL_MISMATCH'
    );
    require(
      wrapper.decimals() == IERC20Metadata(underlying).decimals(),
      'WRAPPER_DECIMALS_MISMATCH'
    );
  }
  ...
  function _scenarioLiquidClaim() internal {
    ...
    uint256 treasuryDelta = treasury == address(0)
      ? 0
      : IERC20(dust).balanceOf(treasury) - treasuryBefore;

    console2.log('userReceived', userDelta);
    console2.log('treasuryReceived', treasuryDelta);
    _assertInstantClaimSplit(claimable, userDelta, treasuryDelta, 'liquidClaim');
  ...
  function _scenarioLockClaimExisting() internal {
    ...
    _advanceTime(warpSeconds);
    (bool okClaimable, uint256 claimable) = _getClaimableRewardsSafe(
      user,
      dust,
      'lockClaim(existing)'
    );
    ...
    bool ok = _callClaimRewardsWithLock(
      user,
      user,
      0,
      existingTokenId,
      'claimRewardsWithLock(existing)'
    );
    if (!ok) {
      statusC = ScenarioStatus.Fail;
      return;
  ...
  function _scenarioClaimToSelf() internal {
    ...
    vm.prank(user);
    try wrapper.claimRewardsToSelf(_singleReward(dust)) {} catch (bytes memory reason) {
      _logRevert('claimRewardsToSelf', reason);
      statusH = ScenarioStatus.Fail;
      return;
    }
    uint256 userDelta = IERC20(dust).balanceOf(user) - userBefore;
    uint256 treasuryDelta = IERC20(dust).balanceOf(treasury) - treasuryBefore;
    console2.log('userReceived', userDelta);
    console2.log('treasuryReceived', treasuryDelta);
    _assertInstantClaimSplit(claimable, userDelta, treasuryDelta, 'claimToSelf');
    console2.log('remainingClaimable', wrapper.getClaimableRewards(user, dust));
  ...
  function _scenarioAuthorizedClaimer() internal {
    ...
    bool setOk = false;
    vm.prank(user);
    (bool okSelf, bytes memory reasonSelf) = address(controller).call(
      abi.encodeWithSignature('setClaimer(address,address)', user, claimer)
    );
    if (!okSelf) {
      _logRevert('setClaimer(self)', reasonSelf);
      if (claimerSetter != address(0)) {
        vm.prank(claimerSetter);
        (bool okAdmin, bytes memory reasonAdmin) = address(controller).call(
          abi.encodeWithSignature('setClaimer(address,address)', user, claimer)
        );
        setOk = okAdmin;
        if (!okAdmin) {
    ...
    uint256 userBefore = IERC20(dust).balanceOf(user);
    uint256 treasuryBefore = IERC20(dust).balanceOf(treasury);
    bool claimedOk = _callClaimRewardsOnBehalfWithLock(
      claimer,
      user,
      user,
      0,
      0,
      'claimRewardsOnBehalfWithLock'
    );
    if (!claimedOk) {
      statusD = ScenarioStatus.Fail;
      return;
    ...
    console2.log('userReceived', userDelta);
    console2.log('treasuryReceived', treasuryDelta);
    _assertInstantClaimSplit(claimable, userDelta, treasuryDelta, 'claimerClaim');
    console2.log('remainingClaimable', wrapper.getClaimableRewards(user, dust));
  ...
  function _scenarioVaultClaimer() internal {
    ...
    vm.startPrank(user);
    try wrapper.transfer(vault, transferAmount) {} catch (bytes memory reason) {
    ...
    try IRewardsControllerEmissionManager(address(controller)).getEmissionManager() returns (
      address manager
    ) {
    ...
    (bool setOk, bytes memory setReason) = address(controller).call(
      abi.encodeWithSignature('setClaimer(address,address)', vault, distributor)
    );
    if (!setOk) {
    ...
    uint256 treasuryBefore = IERC20(dust).balanceOf(treasury);
    bool claimOk = _callClaimRewardsOnBehalfWithLock(
      distributor,
      vault,
      distributor,
      0,
    ...
    console2.log('treasuryReceived', treasuryDelta);
    _assertInstantClaimSplit(claimable, delta, treasuryDelta, 'vaultClaim');
  ...
  function _scenarioZeroBalanceClaim() internal {
    ...
    vm.startPrank(user);
    try wrapper.transfer(user2, userBalance) {} catch (bytes memory transferReason) {
    ...
    console2.log('userReceived', userDelta);
    console2.log('treasuryReceived', treasuryDelta);
    _assertInstantClaimSplit(claimable, userDelta, treasuryDelta, 'zeroBalanceClaim');
    console2.log('remainingClaimable', wrapper.getClaimableRewards(user, dust));
    statusF = ScenarioStatus.Pass;
  }

  function _scenarioCollectToWrapper() internal {
    console2.log('\nScenario G: Legacy collect hook is disabled');
    _advanceTime(warpSeconds);
    ...
    try wrapper.collectAndUpdateRewards(dust) returns (uint256 collected) {
      ...
      uint256 wrapperDelta = IERC20(dust).balanceOf(wrapperAddr) - wrapperBefore;
      uint256 treasuryDelta = treasury == address(0)
        ? 0
        : IERC20(dust).balanceOf(treasury) - treasuryBefore;
      console2.log('wrapperDelta', wrapperDelta);
      console2.log('treasuryDelta', treasuryDelta);
      require(collected == 0, 'COLLECT_NOT_DISABLED');
      require(wrapperDelta == 0, 'COLLECT_MOVED_TO_WRAPPER');
      require(treasuryDelta == 0, 'COLLECT_PENALIZED');
  ...
  function _assertSummary() internal view {
    bool failOnSkip = vm.envOr('FAIL_ON_SCENARIO_SKIP', false);
    _assertScenario(statusA, failOnSkip, 'A');
    _assertScenario(statusB, failOnSkip, 'B');
    _assertScenario(statusC, failOnSkip, 'C');
    _assertScenario(statusD, failOnSkip, 'D');
    _assertScenario(statusE, failOnSkip, 'E');
    _assertScenario(statusF, failOnSkip, 'F');
    _assertScenario(statusG, failOnSkip, 'G');
    _assertScenario(statusH, failOnSkip, 'H');
    _assertScenario(statusI, failOnSkip, 'I');
  }
  ...
  function _assertScenario(
    ...
    require(
      ...
      string(abi.encodePacked('SCENARIO_', label, '_UNKNOWN'))
    );
    require(status != ScenarioStatus.Fail, string(abi.encodePacked('SCENARIO_', label, '_FAILED')));
    if (failOnSkip) {
      require(
        status != ScenarioStatus.Skip,
        string(abi.encodePacked('SCENARIO_', label, '_SKIPPED'))
      );
    }
  }
  ...
  function _assertInstantClaimSplit(
    uint256 claimedAmount,
    uint256 recipientDelta,
    uint256 treasuryDelta,
    string memory label
  ) internal pure {
    // DustLockTransferStrategy direct claims are fixed 50/50, independent of DustLock.earlyWithdrawPenalty().
    uint256 expectedTreasury = claimedAmount / 2;
    uint256 expectedRecipient = claimedAmount - expectedTreasury;
    require(
      recipientDelta == expectedRecipient,
      string(abi.encodePacked(label, '_RECIPIENT_SPLIT'))
    );
    require(treasuryDelta == expectedTreasury, string(abi.encodePacked(label, '_TREASURY_SPLIT')));
  }

  function _callClaimRewards(
    address caller,
    address receiver,
    string memory label
  ) internal returns (bool) {
    vm.prank(caller);
    try wrapper.claimRewards(receiver, _singleReward(dust)) {} catch (bytes memory reason) {
  ...
  function _callClaimRewardsWithLock(
    ...
    vm.prank(caller);
    try wrapper.claimRewardsWithLock(receiver, _singleReward(dust), lockTime, tokenId) {} catch (
      bytes memory reason
    ) {
      _logRevert(label, reason);
      return false;
  ...
  function _callClaimRewardsOnBehalfWithLock(
    ...
    vm.prank(caller);
    try
      wrapper.claimRewardsOnBehalfWithLock(
        onBehalfOf,
        receiver,
        _singleReward(dust),
        lockTime,
  ...
  function _getClaimableRewardsSafe(
    address account,
    address reward,
    string memory label
  ) internal view returns (bool ok, uint256 claimable) {
  ...
  function _deployWrapper() internal returns (address deployed, address selectedAToken) {
    ...
    if (atokenEnv == address(0)) {
      ...
      DataTypes.ReserveData memory data = IPool(NeverlandMonadMainnet.POOL).getReserveData(
        selectedAsset
      );
    ...
    if (proxyAdmin == address(0)) {
      ...
      address proxyAdminOwner = vm.envOr('PROXY_ADMIN_OWNER', vm.addr(uint256(0xB0B)));
