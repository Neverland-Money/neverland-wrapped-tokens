...
contract VerifyDeployment is Script {
  ...
  address constant DEFAULT_EXPECTED_PROXY_ADMIN_OWNER = 0x3e4749D9Df7EC5ecd9184c301592bAc058a6F82f;
  address internal EXPECTED_DEPLOYER;
  address internal EXPECTED_PROXY_ADMIN_OWNER;
  address internal EXPECTED_REWARD_RESCUE_ADMIN;
  ...
  function _loadDeploymentAddresses() internal {
    ...
    TRANSPARENT_PROXY_FACTORY = _envOrAddress(
      'TRANSPARENT_PROXY_FACTORY',
      DEFAULT_TRANSPARENT_PROXY_FACTORY
    );
    STATIC_A_TOKEN_IMPL = _envOrAddress('STATIC_A_TOKEN_IMPL', DEFAULT_STATIC_A_TOKEN_IMPL);
    STATIC_A_TOKEN_FACTORY = _envOrAddress(
      'STATIC_A_TOKEN_FACTORY',
      DEFAULT_STATIC_A_TOKEN_FACTORY
    );
    EXPECTED_DEPLOYER = _envOrAddress('EXPECTED_DEPLOYER', DEFAULT_EXPECTED_DEPLOYER);
    EXPECTED_PROXY_ADMIN_OWNER = _envOrAddress(
      'EXPECTED_PROXY_ADMIN_OWNER',
      DEFAULT_EXPECTED_PROXY_ADMIN_OWNER
    );
    EXPECTED_REWARD_RESCUE_ADMIN = _envOrAddress(
      'EXPECTED_REWARD_RESCUE_ADMIN',
      EXPECTED_PROXY_ADMIN_OWNER
    );
    ...
    _logAddress('EXPECTED_PROXY_ADMIN_OWNER', EXPECTED_PROXY_ADMIN_OWNER);
    _logAddress('EXPECTED_REWARD_RESCUE_ADMIN', EXPECTED_REWARD_RESCUE_ADMIN);
    console.log('');
  }
  ...
  function _verifyProxyAdmin() internal {
    ...
    try proxyAdmin.owner() returns (address owner) {
      _assertEqual(owner, EXPECTED_PROXY_ADMIN_OWNER, 'ProxyAdmin owner');
    } catch {
  ...
  function _verifyStaticToken(
    address underlying,
    address expectedNToken,
    string memory expectedNTokenSymbol
  ) internal {
    ...
    _assertEqual(_readProxyAdmin(staticToken), PROXY_ADMIN, 'Proxy admin');

    try token.REWARD_RESCUE_ADMIN() returns (address rewardRescueAdmin) {
      _assertEqual(rewardRescueAdmin, EXPECTED_REWARD_RESCUE_ADMIN, 'Reward rescue admin');
    } catch {
      _failCall('StaticAToken.REWARD_RESCUE_ADMIN()');
    }
    ...
    try IAToken(aToken).getIncentivesController() returns (address incentivesController) {
      _assertEqual(
        ...
        EXPECTED_REWARDS_CONTROLLER,
        'nToken incentives controller'
      );
    } catch {
